...
**Contact Sample**:
LinkedIn employee profiles fetched from a target company via Apify using the job listing's LinkedIn company page URL (`companyUrl` from Bright Data). Enrichment issues one Apify run per active Contact Search Settings audience toggle — Recruiters only, Russian Speakers only, or both (two runs). Each run uses audience-targeted Apify filters rather than an unfiltered employee page. No name-based slug guessing — if `companyUrl` is missing or a fetch returns zero profiles, Enrichment stops without further Apify calls for that audience. Results are merged by LinkedIn profile URL; the Job Poster is included in classification when present. Contact caps: up to **5** Russian Speakers who are not Recruiters, and up to **3** Recruiters (Recruiter cap applies even when Recruiters is the only active toggle).
_Avoid_: Employee list, full company scrape, unfiltered employee page

**Contact Sample Cache**:
A per-company, per-audience-stream store in the Job Tracker Database of raw Apify profiles for each active filter (`russian` or `recruiters`), keyed by LinkedIn company slug from the job listing's `companyUrl` plus stream. Each stream tracks its own profile list and how many LinkedIn search pages have been fetched so the next Apify call appends the following page for that filter only. Stores successful Apify responses including empty lists — a cached empty stream prevents repeat Apify calls for that filter until **Load More Contacts** requests the next page. Infrastructure failures are not cached. Entries do not expire by age. Pre-migration unfiltered cache entries (single blob per company without a stream key) are treated as a cache miss for all streams — the next enrichment fetches fresh filtered samples rather than reusing legacy data. Outreach Audience classification and Job Poster merge run on every enrichment regardless of cache hit — only Apify fetches are skipped for streams already cached. Cache hits and misses are logged to Task Logs; cache hits are also recorded in the job's Job Activity Log.
...
**Load More Contacts**:
The manual action on **Accepted Jobs** to fetch the next Apify page for audience stream(s) that are still below their contact cap after classification. Only streams with an **active** Contact Search Settings toggle are considered. A stream is loaded when it kept fewer contacts than its cap (5 Russian Speakers who are not Recruiters, 3 Recruiters). If one stream is full and another is short, only the short stream is fetched (one Apify run). If both active streams are short, both are fetched (two Apify runs). The Kanban Dashboard confirms with the user before each run using the same native `confirm()` style as **Enrich Job** — plain-text summary listing only the billable stream(s), profile count per stream, page number, total run count, estimated cost, and “Proceed?”. Appends new profiles to that stream's Contact Sample Cache entry (never replaces earlier pages), then re-runs classification and template generation on the combined cached streams for active toggles. Each page costs one Apify run per stream loaded. There is no page cap — the user may keep loading until Apify returns no further pages for a stream, quotas are met, or they stop. Not shown on **Found Jobs** — first fetch uses **Enrich Job**. Shown on **Accepted Jobs** only after enrichment has run at least once and the job has a `companyUrl` — not on never-enriched Accepted cards. When Outreach Settings change, use **Re-classify** on each job to apply new audience toggles without Apify.
_Avoid_: refresh contacts, bust cache, reload contacts, re-fetch page one, load all streams regardless of quota

**Outreach Audience**:
The classification assigned to a contact by the LLM: Russian Speaker, Recruiter, or both. Determines which Outreach Message Template is loaded and which badges the Kanban Dashboard shows. Badges follow classification flags only (`is_recruiter` → **HR** badge, `russian_speaker` → **RU** badge) — not title keyword matching. When a contact qualifies as both, the Recruiter template takes priority and the contact is grouped under Recruiters — but both audience badges are shown. Dual-classified contacts count toward the **Recruiter** cap only, not the **Russian Speaker** cap; the Russian Speaker pool includes only contacts with `russian_speaker` and not `is_recruiter`.
...
**Enrichment Failure**:
An enrichment run that ends with zero Outreach Audience contacts for any **active** Contact Search Settings toggle, or that encounters an infrastructure error (missing or unusable `companyUrl`, Apify trigger failure, zero employees for a required stream, timeout, missing credentials, classification error). Partial success counts as success — e.g. 3 Recruiters but 0 Russian Speakers when both toggles are on is not a failure; an Enrichment Note may warn that an active stream found no matches and suggest **Load More Contacts**. The job stays in the **Accepted** lane. Even with zero contacts, the Outreach Generator still produces templates for active audience types so the user can cold-connect manually.
