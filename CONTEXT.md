...
**Job Comment**:
User-written notes on a job card, stored in the `comment` field on the Job Tracker Database row. Edited in the job drawer's Notes / Comments textarea. Saved only when the user clicks **Post** — not on every keystroke. **Cancel** reverts to the last **Posted** value. The Kanban card preview shows the last **Posted** **Job Comment** only, not an in-progress **Drawer Draft**. Distinct from **Enrichment Note**, which is system-written and read-only.
_Avoid_: notes field, card notes, user comment

**Drawer Draft**:
An in-progress edit in the job drawer that has not been **Posted** yet — either a **Job Comment** or an **Outreach Message Template** for the **Active Contact**'s audience. Held locally in the textarea until **Post** writes to the Job Tracker Database; **Cancel** reverts to the last **Posted** value. Closing the drawer, navigating prev/next between jobs, switching **Active Contact**, or same-job drawer re-renders with a dirty draft show an **Unsaved Draft Warning** styled like **Spend Confirmation**; **Discard** reverts all dirty drafts and continues, modal **Cancel** keeps edits intact.
_Avoid_: unsaved textarea, draft note, unposted edit

**Unsaved Draft Warning**:
A styled confirmation modal (same shell as **Spend Confirmation**) shown when the user tries to leave the job drawer or replace drawer content while a **Drawer Draft** is dirty. Title **Unsaved changes**, subtitle **Your edits have not been posted.**, body names what would be lost (notes, outreach draft, or both). **Cancel** stays in the drawer with drafts intact; **Discard** reverts dirty fields to their last **Posted** values and completes the pending action.
