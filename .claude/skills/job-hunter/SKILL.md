---
name: job-hunter
description: Use this skill whenever the user mentions searching for jobs, matching or evaluating candidate resumes against jobs, searching company employees/recruiters, generating cover letters, promoting active applications in the SQLite database, or running the job_hunter.py script.
---
...
2. Run the orchestrator pipeline command:
   ```bash
   python3 job_hunter.py --search "<position>"
   ```
3. Verify the CLI output. If successful, the job listings will be saved to the SQLite database (`src/job_tracker.db`) and visible in the local Kanban Dashboard.
